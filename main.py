...
def main():
    ...
    param.CPDIP = np.array(
        [[87680, 217.1, -0.9153, 0.002266, 0, 132.81, 343.15], [107900, -330.13, 0.808, 0, 0, 175.43, 303.15]])
    ...
    conver_IB_130 = (df.values[0, 0] * (df.values[0, 24] / 100 - df.values[0, 9] / 100) - df.values[0, 9] / 100 *
                     df.values[
    ...
    mass_frac_130 = np.array(
        [1 - (conver_IB_130 + conver_IP_130) / (df.values[0, 0] + df.values[0, 3] + df.values[0, 6]),
         (conver_IB_130 + conver_IP_130) / (df.values[0, 0] + df.values[0, 3] + df.values[0, 6])])
    mass_frac_140 = np.array(
        [1 - (conver_IB_140 + conver_IP_140) / (df.values[0, 1] + df.values[0, 4] + df.values[0, 7]),
         (conver_IB_140 + conver_IP_140) / (df.values[0, 1] + df.values[0, 4] + df.values[0, 7])])
    mass_frac_150 = np.array(
        [1 - (conver_IB_150 + conver_IP_150) / (df.values[0, 2] + df.values[0, 5] + df.values[0, 8]),
    ...
    cp_liq_130_mix = np.sum(method.compute_liquid_heat_capacity(T_130_log_mean, param) * mole_frac_R130) / np.sum(
        np.array([56, 50.4875]) * np.array(
            [w_IB_R130, 1 - w_IB_R130])) / 1000
    cp_liq_140_mix = np.sum(method.compute_liquid_heat_capacity(T_140_log_mean, param) * mole_frac_R140) / np.sum(
        np.array([56, 50.4875]) * np.array(
            [w_IB_R140, 1 - w_IB_R140])) / 1000
    cp_liq_150_mix = np.sum(method.compute_liquid_heat_capacity(T_150_log_mean, param) * mole_frac_R150) / np.sum(
        np.array([56, 50.4875]) * np.array(
            [w_IB_R150, 1 - w_IB_R150])) / 1000
